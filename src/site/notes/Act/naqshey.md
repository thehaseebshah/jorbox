---
{"dg-publish":true,"permalink":"/act/naqshey/","dg-note-properties":{"category":"Shabab Tarbiyah Curriculum","name":"Naqshey (Map Drawing)"}}
---


# Naqshey (Map Drawing)

## How to Play
- **Age Limit:** 7+
- **Group Size:** 2+
- **Duration:** 30 minutes
- **Prep Time:** < 5 minutes
- **Difficulty:** Easy
- **Rating:** 4
- **Items Needed:** Mentioned in description

## 1. Objective
To foster teamwork, observation skills, and appreciation for the urban planning of major cities in the Muslim world (past and present). Teams must recreate an accurate map of a city based solely on short-term memory.

## 2. Materials Required
- **Source Maps:** Printed A4 color copies of city maps (Cairo, Karachi, Lahore, Cordoba/Andalus). Note: Ensure maps show main roads and landmarks clearly. Or you can use Google Maps on phones.
- **Drawing Materials:** A4 blank paper (one per team).
- **Stationery:** Pens
- **Timer:** A stopwatch or phone timer.

## 3. Preparation (Instructor)
Print or Screenshot the "Source Maps" before the session.

- **Cairo:** Focus on the Nile River split, Tahrir Square, and the Citadel.
- **Karachi:** Focus on the coastline (Clifton), the Port, and Shahrah-e-Faisal.
- **Lahore:** Focus on the Walled City (circular road), the Fort, and the River Ravi.
- **Cordoba (Andalus):** Focus on the Guadalquivir River, the Roman Bridge, and the Great Mosque.

## 4. Step-by-Step Instructions
### Phase 1: The Briefing (5 Minutes)
Gather the Shabab and explain the scenario: You are Muslim travelers and cartographers. You have been given access to the royal archives for exactly 5 minutes. You must memorize the layout of the city to guide your caravan safely. Precision is key as getting lost means failure.

Explain the **Winning Criteria**:

- **Orientation:** Where is North?
- **Scale:** Are the distances realistic?
- **Key Landmarks:** Are the Masjids, Forts, and Rivers in the right place?

### Phase 2: The Study (5 Minutes)
- Divide Shabab into groups of 3.
- Hand out one Source Map face-down to each group.
- Each group gets the same map.
- On the command "GO", groups turn over the map.
- **Strategy Time:** Encourage them to split the work (e.g., Member A memorizes the river, Member B memorizes the roads, Member C memorizes the landmarks).
- After exactly 5 minutes, shout "TIME" and collect all Source Maps immediately.

### Phase 3: The Drawing (15 Minutes)
- Hand out the blank A3 paper and pencils.
- Groups must draw the map from memory.
- They should label roads and landmarks if they remember the names.
- **Instructor Note:** Walk around and encourage them, but do not give hints. Give a "2-minute warning" before the end.

### Phase 4: The Reveal (10 Minutes)
- Ask groups to stick their drawings on the wall.
- Tape the original Source Map next to their drawing for comparison.
- Judge the maps using the scoring rubric below.

## 5. Scoring Rubric (Max Score: 20)
| Criteria | Points | What to look for |
|
---|
---|
---|
| Structure & Flow | 5 | Did they get the shape of the river/coastline or main city wall correct? |
| The "Anchor" | 5 | Is the main road/artery placed correctly? (e.g., The Nile in Cairo, Mall Road in Lahore). |
| Landmarks | 5 | Are specific buildings (Mosques, Forts) in the correct relative location? |
| Scale & Cleanliness | 5 | Is the map readable? Are the proportions realistic? |

**Bonus:** Award +2 points for correctly naming specific neighborhoods or streets.

## 6. Instructor Cheat Sheet (Key Features to Look For)
When judging, look for these specific details in their drawings:

- **Cairo:**
  - The Nile River runs South to North.
  - There are islands in the middle of the river (Gezira).
  - The Pyramids (if shown) are to the West (Giza).
- **Lahore:**
  - The Walled City is roughly circular/kidney-shaped.
  - The Fort is on the Northern edge of the Walled City.
  - The River Ravi flows nearby.
- **Karachi:**
  - The Arabian Sea is to the South.
  - The Port/Kemari is a distinct peninsula sticking out.
  - Main roads run generally away from the port.
- **Cordoba (Andalus):**
  - The river runs right in front of the Great Mosque.
  - The bridge crosses the river directly to the Mosque.
  - The streets are very winding and narrow (maze-like).
